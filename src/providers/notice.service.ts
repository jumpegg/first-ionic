...
export class NoticeService{
	...
	create(input){
		return this.http.post('/api/study/notice', input, {headers:this.header})
			.map(res=>res.json());
	}
	getCnt(){
	...
	update(input){
		return this.http.put('/api/study/notice', input, {headers:this.header})
			.map(res=>res.json());
	}
	delete(input){
		return this.http.delete('/api/study/notice/'+input, {headers:this.header})
			.map(res=>res.json());
	}
	forIndex(){
